...
<head>
    ...
    <?php 
    // getting php variables set in register 
$Name =$_POST['name'];
$Age =$_POST['age'];
$Add =$_POST['add'];
$Pas =$_POST['pwd'];
...
<script typr="text/javascript">
  ...
  if(s==1)
  {
  alert("Login Successful- You may proceed to Payment!!");
  return true;
  }
...
<?php
// php function to set session variables to be used in details.php
    $key=$_SESSION["value"];
    $con = mysqli_connect("localhost","root","");
if (!$con)
{
...
mysqli_select_db($con,"car-rental");
$result = mysqli_query($con,"SELECT C.car_model,C.car_type,C.car_cap,P.payment,P.gst,L.city,L.location FROM cars C,center L,payment P WHERE C.pid=P.pid and P.center_id=L.center_id and C.car_id=$key ");

$row = mysqli_fetch_array($result);
$_SESSION["model"]=$row['car_model'];
$_SESSION["type"]=$row['car_type'];
$_SESSION["cap"]=$row['car_cap'];
$_SESSION["pay"]=$row['payment'];
